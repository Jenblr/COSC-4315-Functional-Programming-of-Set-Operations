...
l_removePeriods = lambda item: [item] if '.' not in item or l_isNum(item) else [item.split('.', 1)[0]] + l_removePeriods(item.split('.', 1)[1]) if item.count('.') == 1 else [item.split('.', 1)[0] + '.' + item.split('.', 1)[1].split('.', 1)[0]] + l_removePeriods(item.split('.', 1)[1].split('.', 1)[1])
